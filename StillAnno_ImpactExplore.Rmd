...
In total 5985 images were potentially targeted for annotation, of these `r nrow(PCcoverbyImage)` have been annotated for percent cover of substrate types, and `r nrow(NSNR)` were identified as unsuitable for annotation and could not be replaced. 
...
kable(Target[1:5], caption="By operation processing summary")
...
```{r FigSMImpact, out.width='100%', fig.height= 20, fig.width=10, fig.align='center', fig.cap="Overview of impact scores over depth for each seamount"}
...
Distribution of the substrate types by the locations mentioned above - here the seamounts from the recovery time series are selected. 

```{r FigPCpies, fig.cap="**Pie charts of substrate distribution at selected locations"}
