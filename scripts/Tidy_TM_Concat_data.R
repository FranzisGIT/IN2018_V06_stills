...
OverviewScores <-  TM_data %>% 
  ...
  mutate(ImpGroup = case_when(
      L2_Code == "N/A" ~"0",
      L2_Code == "High - reef" ~"8",
      L2_Code == "Low - sediment filled" ~"9",
      L2_Code == "Isolated fragments/ clumps" ~"5",
      L2_Code == "MedAb - with clumps - dead" ~"2",
      L2_Code == "HighAb - with patch - live" ~"4",
  ...
  mutate(ImpClass = case_when(
    ...
    L2_Code == "Low - sediment filled" ~"9M",
    L2_Code == "Isolated fragments/ clumps" ~"5R",
    L2_Code == "MedAb - with clumps - dead" ~"2R_M",
    L2_Code == "HighAb - with patch - live" ~"4R",
