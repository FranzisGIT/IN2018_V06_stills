...
```{r FigSubstDpth, fig.cap="Depth distribution of substrate types summary graph"}
...
```{r FigPC_DpthDist2, fig.cap="Depth distribution of substrate types"}
PC_cover %>% 
  ggplot(aes(x = depth,
             y = PC_cover))+
  geom_point(alpha=0.2)+
...
```{r FigPC_pies, eval=FALSE, fig.cap="**Pie charts of substrate distribution at selected locations"}
...
```{r FigSV_histogram, fig.cap="Frequency distrbution histograms of % cover recorded for coral matrix and rubble"}
CoralPC <- PC_cover %>% 
...
```{r FigRubble_checks, fig.cap="Distribution of coral rubble by seamount location"}
PC_cover %>% 
...
```{r, FigPCcover_explore,  fig.cap="*Frequency distribution of a) total density of VME data and b) total number of taxa "}
...
```{r, FigDensity_explore,  fig.cap="**Frequency distribution of total density of VME taxa only "}
# looking at the distribution of density and number of taxa over the whole data set
...
```{r FigPCcoverComparison, fig.cap="Comparison of estimated and measured percent cover of matrix"}
...
estVmeas_lm
```
```{r MyFig4, fig.align='center', fig.height= 4, fig.width=6,  fig.cap="Relationship between density of VME taxa and percent cover of matrix forming coral"}
VMEanno_DensQ %>% 
...
```{r FigImpactdata, fig.cap="Impact overview evaluation"}
PCcoverbyImage %>% 
  ggplot(aes(x = depth,
             y = `SU-BCOR`,
             colour = OV_CAT))+
