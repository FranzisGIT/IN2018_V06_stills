...
```{r by depth distribution, fig.cap="**Figure 1** depth distribution of substrate types summary graph"}
...
```{r SV data depth, fig.cap="**Figure 2** depth distribution of substrate types"}
PC_cover %>% 
  ggplot(aes(x = depth,
             y = PC_cover))+
  geom_point(alpha=0.2)+
...
```{r PC cover pies, fig.cap="**Figure 3** pie charts of substrate distribution at selected locations"}
Fang <- PC_cover %>% 
  filter(MapLoc=="Fang") %>% 
  ggplot(Fang,
       mapping= aes(x="", 
...
```{r SV data histogram, fig.cap="**Figure 4** Frequency distrbution histograms of % cover recorded for coral matrix and rubble"}
CoralPC <- PC_cover %>% 
...
```{r rubble checks, fig.cap="**Figure 5** distribution of coral rubble by seamount location"}
PC_cover %>% 
